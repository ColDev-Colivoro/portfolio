const ChimubotAvatar = ({ isOpen, isLoading, isVisible, onToggle, lang = 'es' }) => {
	...
	return (
		...
		>
			<span className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-accent/25 bg-accent/15 text-[11px] font-semibold text-accent">
				C
			</span>
			<span>{isOpen ? (lang === 'es' ? 'Abierto' : 'Open') : lang === 'es' ? 'Abrir' : 'Open'}</span>
