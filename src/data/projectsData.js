...
export const projectsCatalog = [
	...
	},
	{
		...
		title: {
			es: 'Nutrisco Standardization',
			en: 'Nutrisco Standardization',
		},
		subtitle: {
			es: 'Plataforma de Diálogo de Desempeño Industrial',
			en: 'Industrial Performance Dialogue Platform',
		},
		summary: {
			es: 'Sistema integral para estandarizar procesos operativos en plantas de producción de Nutrisco/Orizon: registra indicadores, calcula KPIs con fórmulas configurables, conduce Diálogos de Desempeño (reuniones estructuradas) y hace seguimiento a compromisos correctivos en un Kanban multi-rol.',
			en: 'Comprehensive system to standardize operational processes at Nutrisco/Orizon production plants: logs indicators, computes KPIs via configurable formulas, runs structured Performance Dialogues (meetings), and tracks corrective commitments in a multi-role Kanban board.',
		},
		problem: {
			es: 'Las plantas operarían sin visibilidad compartida: los indicadores de producción se registraban en papel o planillas dispersas, las reuniones no tenían estructura ni bitácora, y los compromisos acordados se podían perder sin seguimiento. La mejora continua no era trazable.',
			en: 'Plants operated without shared visibility: production indicators were logged on paper or scattered spreadsheets, meetings had no structure or minutes, and agreed-upon commitments could be lost without tracking. Continuous improvement was not auditable.',
		},
		impact: {
			es: 'Se centralizó la operación en una sola plataforma: colaboradores ingresan datos por turno, el motor de fórmulas calcula KPIs y genera semáforos automáticos, y los líderes conducen Diálogos de Desempeño en 3 capas jerárquicas con actas digitales que generan compromisos rastreables (responsable + fecha límite).',
			en: 'The operation was centralized into a single platform: collaborators input data per shift, the formula engine auto-calculates KPIs and generates traffic-light alerts, and leaders run Performance Dialogues at 3 hierarchical levels with digital minutes that spawn trackable commitments (owner + deadline).',
		},
		role: {
			es: 'Desarrollo Full Stack End-to-End: diseño del modelo de datos relacional (PostgreSQL), API REST modular (Django 5 + DRF), frontend reactivo (Next.js 15 App Router, React 19, TypeScript), flujo de autenticación JWT (SimpleJWT), e infraestructura Docker (Backend + Frontend + DB) con proxy Nginx.',
			en: 'End-to-End Full Stack Development: relational data model design (PostgreSQL), modular REST API (Django 5 + DRF), reactive frontend (Next.js 15 App Router, React 19, TypeScript), JWT auth flow (SimpleJWT), and Docker infrastructure (Backend + Frontend + DB) with Nginx proxy.',
		},
		stack: ['Django 5.2', 'Next.js 15', 'React 19', 'TypeScript', 'PostgreSQL 15', 'Docker', 'SimpleJWT', 'Gunicorn', 'Nginx'],
		phase: 'mvp',
		access: 'corporate',
		links: {
		...
		caseStudy: {
			headline: {
				es: 'Nutrisco — Sistema de Mejora Continua para Plantas de Producción',
				en: 'Nutrisco — Continuous Improvement System for Production Plants',
			},
			description: {
				es: 'Arquitectura de 3 capas Dockerizada: API REST modular en Django 5 + DRF sirviendo 6 apps (core, usuarios, indicadores, kpis, diálogos, compromisos) sobre PostgreSQL 15, servida por Gunicorn detrás de Nginx. Frontend Next.js 15 con App Router y React 19 consume la API con rutas protegidas por rol. Autenticación mediante SimpleJWT + Firebase Refresh. El Motor de Fórmulas de KPIs evalúa expresiones configurables (ej: A/B) resolviendo operandos recursivos desde indicadores reales registrados por turno.',
				en: 'Dockerized 3-layer architecture: modular Django 5 + DRF REST API serving 6 apps (core, users, indicators, kpis, dialogues, commitments) on PostgreSQL 15, served by Gunicorn behind Nginx. Next.js 15 App Router frontend with React 19 consumes the API via role-protected routes. Authentication via SimpleJWT + Firebase Refresh. The KPI Formula Engine evaluates configurable expressions (e.g. A/B) resolving recursive operands from real indicators logged per shift.',
			},
			microStats: {
				es: ['4 Roles de Usuario', '6 Módulos Core', 'Motor de Fórmulas KPI'],
				en: ['4 User Roles', '6 Core Modules', 'KPI Formula Engine'],
			},
			features: {
				es: [
					'3 tipos de Diálogo por capa jerárquica: Estratégico (Gerencia), Táctico (Jefaturas de Planta) y Operativo (Supervisores de turno) — cada uno filtra sus propias métricas y compromisos.',
					'Motor de KPIs: fórmulas configurables visualmente (ej: Producción / Horas Hombre) con semáforos automáticos (Verde/Rojo) evaluados contra meta y rango.',
					'Wizard de Diálogo: asistente paso a paso para moderar la reunión, generar actas digitales inmutables y definir temas (KPIs, indicadores, compromisos previos).',
					'Kanban de Compromisos: ciclo de vida Pendiente → En Proceso → Atrasado → OK, con asignación de responsable, fecha límite y hilo de comentarios por tarea.',
				],
				en: [
					'3 Dialogue types per hierarchy layer: Strategic (Management), Tactical (Plant Leads), and Operational (Shift Supervisors) — each filters its own metrics and commitments.',
					'KPI Engine: visually configurable formulas (e.g. Production / Man-Hours) with auto traffic-light alerts (Green/Red) evaluated against goal and range.',
					'Dialogue Wizard: step-by-step assistant to moderate the meeting, generate immutable digital minutes, and define agenda items (KPIs, indicators, prior commitments).',
					'Commitments Kanban: lifecycle Pending → In Progress → Overdue → Done, with owner assignment, deadlines, and a comment thread per task.',
				],
			},
			credentials: {
				es: [
					{ label: 'Backend', text: 'Django 5.2 + Django REST Framework + Gunicorn' },
					{ label: 'Frontend', text: 'Next.js 15 App Router + React 19 + TypeScript + Tailwind' },
					{ label: 'Base de datos', text: 'PostgreSQL 15 — 6 apps relacionales, > 15 modelos' },
					{ label: 'Despliegue', text: 'Docker Compose (3 servicios) + Nginx proxy inverso' },
				],
				en: [
					{ label: 'Backend', text: 'Django 5.2 + Django REST Framework + Gunicorn' },
					{ label: 'Frontend', text: 'Next.js 15 App Router + React 19 + TypeScript + Tailwind' },
					{ label: 'Database', text: 'PostgreSQL 15 — 6 relational apps, > 15 models' },
					{ label: 'Deployment', text: 'Docker Compose (3 services) + Nginx reverse proxy' },
				],
			},
			credentialsNote: {
